...
function Show-Help {
    ...
    Write-Host "Bookkeeping API Tools"
    Write-Host ""
    Write-Host "A command-line tool for calling Bookkeeping APIs"
    Write-Host ""
    Write-Host "Usage:"
    ...
    Write-Host "    EBKTOOL_SERVER_BASEURL      Bookkeeping server base URL (e.g., http://localhost:8080)"
    Write-Host "    EBKTOOL_TOKEN               Bookkeeping API token"
    Write-Host ""
...
function Invoke-Api {
    ...
    if (-not $serverBaseUrl) {
        ...
        Write-Host "Please set it to your Bookkeeping server base URL (e.g., http://localhost:8080)"
        exit 1
    }
